...
public class LambdaRuntime {
    ...
    private static final String X_AMZN_TRACE_ID = "Root=1-dc99d00f-c079a84d433534434534ef0d;Parent=91ed514f1e5c03b2;Sampled=0";
    ...
    static {
        ...
        HANDLER = args.length > 1 ? args[1] : getEnvOrDefault("AWS_LAMBDA_FUNCTION_HANDLER", "index.Handler");
    ...
    public static void reportDone(final String invokeid, final byte[] result, final int resultLength, final int p3) {
        ...
        systemLog(String.join("\t", "REPORT RequestId: " + invokeid,
                "Duration: " + String.format("%.2f", durationMs) + " ms", "Billed Duration: " + billedMs + " ms",
                "Memory Size: " + MEMORY_LIMIT + " MB",
                "Max Memory Used: " + Math
                        .round((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / (1024 * 1024))
                        + " MB",
                ""));
        if (result != null) {
